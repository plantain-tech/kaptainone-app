...
$booking = $bookingId > 0 ? Database::fetch(
    "SELECT b.*, l.title AS listing_title, l.asset_type, l.location_district, l.description,
     ...
     LEFT JOIN user_profiles p ON p.user_id = b.owner_user_id
     WHERE b.id = ?",
    [$bookingId]
) : null;

if (!$booking || ((int) $booking['courier_user_id'] !== (int) $user['id'] && !has_role('admin'))) {
...
$ownerFirst = trim((string) ($booking['owner_name'] ?? ''));
$ownerFirst = $ownerFirst !== '' ? strtok($ownerFirst, ' ') : 'Verified owner';
...
<section class="booking-detail-shell">
    ...
    <?php if (in_array($booking['status'], ['requested', 'approved'], true)): ?>
        <form class="booking-detail-actions" method="post" action="<?= base_url() ?>/dashboard/courier/booking-action.php" onsubmit="return confirm('Are you sure you want to cancel this booking request? The owner will be notified.');">
    ...
    <div class="booking-detail-grid">
        ...
        <section class="booking-info-card">
            <span class="section-kicker">Pickup contact</span>
            <h3>Owner and pickup</h3>
            <div class="booking-party-list">
                <div><span>Owner</span><strong><?= e($ownerFirst) ?> <em>Verified owner</em></strong></div>
                <div><span>Pickup district</span><strong><?= e($booking['location_district']) ?></strong></div>
                <div><span>Listing</span><a href="<?= base_url() ?>/listing.php?id=<?= (int) $booking['listing_id'] ?>">View full listing details</a></div>
            </div>
        </section>
    </div>
    ...
    <section class="booking-info-card">
        ...
        <?php if ($booking['courier_message']): ?>
            <div class="booking-message-block">
                <strong>Your message to the owner</strong>
